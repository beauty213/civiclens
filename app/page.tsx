...
export default async function HomePage() {
  ...
  const reports = SHOW_SECONDARY_HOME_SECTIONS ? await fetchCitizenReports() : [];
  const questions = SHOW_SECONDARY_HOME_SECTIONS ? await fetchCivicQuestions() : [];
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {sortedArticles.length > 0 ? (
        <StoryPulseFilters articles={sortedArticles} />
      ) : (
        <section aria-live="polite" className="rounded-xl border border-dashed border-zinc-800 bg-zinc-900/40 p-8 text-center">
          <h2 className="text-sm font-medium text-zinc-200">No articles to show yet</h2>
          <p className="mt-1 text-xs text-zinc-500">New stories will appear here when they are available.</p>
        </section>
      )}
